...
function onFile(e: Event){ const target = e.target as HTMLInputElement; file.value = target.files?.[0] || null }
async function submit(){
  ...
  try{
    ...
    const { error: dbError } = await supabase.from('payment_submissions').insert({ reference_no, full_name:form.full_name, phone:form.phone, email:form.email, payment_date:form.payment_date, proof_file_path:path })
    ...
    await downloadSlip()
    Object.assign(form,{ full_name:'', phone:'', email:'', payment_date:'', confirm:false }); file.value=null
        ...
        <section class="card">
          ...
          <form @submit.prevent="submit">
            <div class="form-grid">
              ...
              <div class="field full"><label>Proof of Payment <span>*</span></label><div class="drop"><p><b>Click to upload or drag and drop</b><br/><small>JPG, PNG, PDF. Max 10MB</small></p><input type="file" accept="image/*,.pdf" @change="onFile" /></div></div>
            </div>
